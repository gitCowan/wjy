...
{    
    ...
    public function  __construct() {   
        ...
        $config_value = M('pay')->where(array('type'=>'Wxpay'))->find();    
        WxPayConfig::$appid = $config_value['appid']; // * APPID：绑定支付的APPID（必须配置，开户邮件中可查看）
        WxPayConfig::$mchid = $config_value['pid']; // * MCHID：商户号（必须配置，开户邮件中可查看）
...
// dump(WxPayConfig::$appid);die;		
    ...
    {       
            ...
            return '<img alt="模式二扫码支付" src="/index.php?m=Home&c=Index&a=erweima&data='.urlencode($url2).'" style="width:110px;height:110px;"/>';        
    }    
    /**
     * 服务器点对点响应操作给支付接口方调用
     * 
    ...
    function getJSAPI($order){
        $go_url = U('Home/User/memberinfo');
        $back_url = U('Home/User/memberinfo');
        //①、获取用户openid
        $tools = new JsApiPay();
        $uid = $_SESSION['uid'];
        $openId = M('userinfo')->where(array('uid'=>$uid))->find();
        $openId = $openId['openid'];
			// dump($openId);die;
        //②、统一下单
        $input = new WxPayUnifiedOrder();
        $input->SetBody("支付订单：".$order['bpno']);
        $input->SetAttach("weixin");
        $input->SetOut_trade_no($order['bpno'].time());
        $input->SetTotal_fee($order['bpprice']*100);
