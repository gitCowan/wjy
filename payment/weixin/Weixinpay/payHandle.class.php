...
public $token; 
	...
	$this->from=$from;
	 ...
	 $this->from=$this->from!='groupon'?$this->from:'Groupon'; 
		...
		case 'store': $this->db=M('store_cart'); 
		break; case 'repast': 
		...
		break; case 'card': 
		$this->db=M('Member_card_pay_record'); break; 
		} 
		$this->token=$token; $this->payType=$paytype; 
		 if($this->from == 'Jiudian'){$this->db=M('hotels_order');}
