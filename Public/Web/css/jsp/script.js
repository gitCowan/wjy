...
function  abc(code){
		...
		url = 'http://'+document.domain+'/index.php/Home/Index/x';
	  //url ='/xh/tu.php';
	 //url="http://m.kxt.com/hQuotes/chart?code=";
	
	  $('.TimeMenu li a').click(function(){
		 
		  if( $('.TimeMenu li a.curr').attr('data-type') != $(this).attr('data-type') || $(this).attr('data-type')=='area' ){
			  $('.TimeMenu li a.curr').removeClass('curr');
			  $(this).addClass('curr');
			  get_url(1);
		  }else {
	  ...
	  get_url = function(is_iframe){
		  rows=45;
		  interval = $('.TimeMenu li a.curr').attr('data-interval');
		  
		  type = $('.TimeMenu li a.curr').attr('data-type');
		  // type = $('.Linemenu li.curr').attr('data-type');
		  if( !type ){
			  type = 'candlestick';
		  }
		  ...
		  if(is_iframe==1){
			  ...
			  $('.trend-chart').html('<iframe id="chart" class="efc" width="100%"  scrolling="no" frameborder="0" top="100" src="'+ url +'/'+  parameter_str +'"></iframe>');
		  }
        ...
        success: function(data) { 
			if(data){
				for(i=0;i<data.length;i++){
					$(".yincangyoujia_"+data[i].oid).html(data[i].price);
				}  
			}            
        },  
    }); 
