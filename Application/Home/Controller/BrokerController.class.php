...
class BrokerController extends CommonController {
    ...
    public function applybroker(){
		if(IS_POST){
            ...
            if ($obj['agenttype']==0) {
                ...
                $data['mname']=I('post.username');
                $data['vid'] = $user['vid'];
                $manager=M('managerinfo')->where('uid='.$data['uid'])->count();
    ...
    public function brokerinfo(){
        ...
        $tq=C('DB_PREFIX');
        $user = M('userinfo');
        $field = $tq.'userinfo.uid as uid,'.$tq.'userinfo.username as username,'.$tq.'userinfo.utel as utel,'.$tq.'managerinfo.mname as mname,'.$tq.'userinfo.otype as otype,'.$tq.'userinfo.agenttype as agenttype,'.$tq.'userinfo.portrait as portrait';
    ...
    public function mytiny(){
        ...
        if($users['agenttype']==2){
            ...
            $aee['uid'] = $uid;
            $aee['jtype'] = '返点';
            $jour = $jo->field('jfee')->where($aee)->select();
            $jaccess_sum = 0;
            foreach($jour as $k => $v){     
                $jaccess_sum += $v['jfee'];
            }
    ...
    public function dhold(){
        ...
        //根据登录id查询商户所有的下线id
        $arruid = array();
        $userlist=$userinfo->field('uid,username')->where('vid='.$myuid)->select();
