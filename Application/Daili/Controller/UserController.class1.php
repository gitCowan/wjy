...
class UserController extends Controller {
	 	...
	 	if(IS_POST){
			...
			if(empty($result)){
				...
				if($result['upwd'] == md5(I('post.password'))){
					...
					if($result['otype']==1 && $result['ustatus']==0)
					{    
					    session('did',$result['uid']);
					    //记住用户名
    ...
    */
    public function signinout()
    {
		...
		$this->success('正在退出登录...',U('Daili/User/signin'));
    }
	...
	public function ulist(){
		...
		$ulist   = $user->order('uid desc')->select();
	...
	public function recharge(){
		...
		if($step == "sxsearch"){
			...
			$where = $key.$formula;
			$rechargelist = $balance->where($where)->join($tq.'userinfo on '.$tq.'balance.uid='.$tq.'userinfo.uid','left')->join($tq.'accountinfo on '.$tq.'balance.uid='.$tq.'accountinfo.uid','left')->field($field)->where($where)->order($tq.'balance.bpid desc')->select();
			
			foreach($rechargelist as $k => $v){
				$rechargelist[$k]['bptime'] = date("Y-m-d H:i",$rechargelist[$k]['bptime']);
				if($rechargelist[$k]['cltime']==""){
					$rechargelist[$k]['cltime']="";
				}else{
					$rechargelist[$k]['cltime'] = date("Y-m-d H:i",$rechargelist[$k]['cltime']);	
				}
			}
			if($rechargelist){
	        ...
	        $page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% ');
	        $show = $page->show();
			$rechargelist = $balance->where($where)->limit($page->firstRow.','.$page->listRows)->order('bpid desc')->select();
			foreach($rechargelist as $k=>$v){
				$rechargelist[$k]['username'] = M('userinfo')->where(array("uid"=>$v['uid']))->getfield('username');
				$rechargelist[$k]['balance'] = M('accountinfo')->where(array("uid"=>$v['uid']))->getfield('balance');
			}
	...
	public function checklogin()
	{
		...
		if(!$uid)
		{
		    $this->error('请登录','/index.php/Daili/User/signin');
		}
	}
