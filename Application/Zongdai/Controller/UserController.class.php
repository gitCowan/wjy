...
class UserController extends Controller {
    ...
    */
    ...
    public function recharge(){
        ...
        $rechargelist = $balance->limit($page->firstRow.','.$page->listRows)->where($where)->order($tq.'balance.bpid desc')->select();


        foreach($rechargelist as $k => $v) {
            $user_info = $userinfo -> where('uid='.$rechargelist[$k]['uid']) -> find();
            $account_info = $accountinfo -> where('uid='.$user_info['uid']) -> find();
            $rechargelist[$k]['chongzhi'] = $balance->where('uid='.$user_info['uid'].' and bptype="充值"'.' and remarks like "充值成功-%"')->sum('bpprice');
            $rechargelist[$k]['tixian'] = $balance->where(array('uid'=>$user_info['uid'],'bptype'=>'提现','isverified'=>'1'))->sum('bpprice');
            $rechargelist[$k]['username'] = $user_info['username']==null ? '该用户已删除' :$user_info['username'];
            $rechargelist[$k]['balance'] = $account_info['balance']==null ? '0.00' :$account_info['balance'];
        }

        $request_list[uid]=$uid;
        $request_list[bptype]=$bptype;
