...
class CouponsController extends Controller {
    public function cpadd()
	{
		...
		/*	$data['gettime'] = strtotime(I('post.gettime'));
			...
			if($exper->create()){
				...
				if($result){
					$this->success("体验劵添加成功");
				}else{
					$this->error('体验劵添加失败，请重新添加！');
				}
			}else{
    ...
    public function cpsend()
    {
        ...
        if(IS_POST){
			...
			//print_r($experienceinfo->create());die;
			...
			if($result){
				$this->success("体验劵发放成功",U('Coupons/cplist',array('style'=>'list')));
			}else{
				$this->error('体验劵发放失败，请重新添加！');
			}
