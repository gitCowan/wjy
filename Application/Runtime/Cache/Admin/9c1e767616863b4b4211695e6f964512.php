            ...
            <div class="panel" id="topheader">
                <header>
                    <table>
                        <tbody>
                        <tr>
                            <th style="width:40%;"><h4 style="font-size:20px;color:#8fb300">产品分类</h4></th>
                            <th style="width:60%;" class="col-md-12">

                                <div class="col-md-12">
                                    <a href="<?php echo U('Goods/gtypeadd');?>" type="button" style="float:right" class="btn btn-success">添加分类</a>
                                </div>
                ...
                <div id="detailsTable" style="">
                    <table class="table table-hover table-striped">
                        ...
                        <?php if(is_array($typelist)): $i = 0; $__LIST__ = $typelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gl): $mod = ($i % 2 );++$i;?><tr>
                                <td><?php echo ($gl['cid']); ?></td>
                                <td><?php echo ($gl['cname']); ?></td>
                                <td><?php echo ($gl['title']); ?></td>
                                <td><?php if($gl["status"] == 0): ?>已关闭<?php else: ?>已开启<?php endif; ?></td>
                                <td style="text-align:center;">
                                    <li style="border:0;float:left;margin-right:3px;"><a href="<?php echo U('Goods/api_list',array('cid'=>$gl['cid']));?>" type="button" class="btn btn-primary btn-sm">时实数据</a></li>
                                    <?php if($gl["status"] == 0): ?><a href="<?php echo U('Goods/gtedit',array('cid'=>$gl['cid'],'status'=>$gl['status']));?>" onclick="if(confirm('确定要开启吗?')){return true;}else{return false;}" type="button" class="btn btn-primary">开启</a>
                                        <?php else: ?>
                                        <a href="<?php echo U('Goods/gtedit',array('cid'=>$gl['cid'],'status'=>$gl['status']));?>" onclick="if(confirm('确定要关闭吗?')){return true;}else{return false;}" type="button" class="btn btn-danger">关闭</a><?php endif; ?>
