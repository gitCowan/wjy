...
<div class="panel" id="topheader">
    <header>
        <table>
            <tbody>
            <tr>
                <th style="width:40%;"><h4 style="font-size:20px;color:#8fb300">自定义菜单</h4></th>
				<th style="width:60%;" class="col-md-12">
				
					<div class="col-md-12">
					<a href="<?php echo U('Menber/diyadd');?>" type="button" style="float:right;margin-left
					:15px;"  class="btn btn-success">添加菜单</a>
					 <a href="<?php echo U('Menber/diy_send');?>" type="button" style="float:right" class="btn btn-warning">生成菜单</a> 
				</div>
...
<div class="panel">
    ...
    <div id="detailsTable" style="">
        <table class="table table-hover table-striped">
           ...
           <tbody>
		   <?php if(is_array($class)): $i = 0; $__LIST__ = $class;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$diy): $mod = ($i % 2 );++$i;?><tr>
			<td><?php echo ($diy['sort']); ?></td>
			<td <?php if($diy['pid'] != 0 ): ?>style="padding-left:40px;"<?php endif; ?> ><?php if($diy['pid'] != 0 ): ?>[子菜单]<?php endif; echo ($diy['title']); ?></td> 
			<td><?php echo ($diy['keyword']); ?></td> 	
			<td><?php echo ($diy['url']); ?></td> 			
			<td style="text-align:center;"><a href="<?php echo U('Menber/diyadd',array('pid'=>$diy['id']));?>" type="button" class="btn btn-primary">修改</a>
				<a href="<?php echo U('Menber/diy_del',array('id'=>$diy['id']));?>" onclick="if(confirm('确定要删除吗?')){return true;}else{return false;}" type="button" class="btn btn-danger">删除</a></td>
