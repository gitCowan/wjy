    ...
    <header>
        <table>
            ...
            <tr>
				...
				<div class="col-md-8">
				<form method="post" action="<?php echo U('Goods/glist');?>">
				...
				<div class="col-md-4">
					<a href="<?php echo U('Goods/gadd');?>" type="button" style="float:right;margin-left
					:15px;"  class="btn btn-success">添加商品</a>
					<!-- <a href="<?php echo U('Goods/gtypeadd');?>" type="button" style="float:right" class="btn btn-warning">添加分类</a> -->
				</div>
...
<div class="panel">
    ...
    <div id="detailsTable" style="">
        <table class="table table-hover table-striped">
           ...
           <tbody>
		   <?php if(is_array($goodlist)): $i = 0; $__LIST__ = $goodlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gl): $mod = ($i % 2 );++$i;?><tr>
			
			<td><a href="<?php echo U('Goods/gedit',array('pid'=>$gl['pid']));?>" ><?php echo ($gl['ptitle']); ?></a></td> 
			<td> <font color="#f00" size="2"><?php echo ($gl['uprice']); ?>元</font></td> 
			<td><?php echo ($gl['wave']); ?></td>  
			<td><?php echo ($gl['feeprice']); ?></td>  
			<td><?php echo ($gl['num']); ?></td>  
			<td ><em class="<?php echo ($gl['title']); ?>"><?php echo ($gl['price']); ?></em>
			</td> 
			<td><?php echo ($gl['cname']); ?></td> 		
			<td style="text-align:center"><li style="border:0;float:left;margin-right:3px;"><a href="<?php echo U('Goods/gedit',array('pid'=>$gl['pid']));?>" type="button" class="btn btn-primary btn-sm">编辑</a></li>
			<li class="last"><a href="<?php echo U('Goods/gdel',array('pid'=>$gl['pid']));?>" onclick="if(confirm('确定要删除吗?')){return true;}else{return false;}" type="button" class="btn btn-danger btn-sm">删除</a></td>
