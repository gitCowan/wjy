...
.new_id2 input{background:none;}

.new_ids1{background:none;}
.new_ids2{background:#111;}
.new_ids3{background:none;}
.new_ids4{background:none;}
.new_ids2 input{background:none;}
  ...
  <div class="index new_inde<?php echo ($tpl); ?>" >
    <header class="list-head new_id<?php echo ($tpl); ?>">
      	<nav class="list-nav clearfix"> <a href="javascript:history.go(-1)" class="list-back"></a>
        	<h3 class="list-title">银行卡提现</h3>
      	</nav>
    </header>
    <form id="withDraw" class="i-form" method="post" action="<?php echo U('User/cash');?>" onSubmit="return false;">
      	<ul class="form-box new_ids<?php echo ($tpl); ?>">
	        <li class="f-line clearfix">
	         	<label class="f-label">可提取余额</label>
	         	<em><?php if($totle["balance"] == ''): ?>0 <?php else: echo ($totle["balance"]); endif; ?>元</em>
	          	<input type="hidden" name="counts" value="<?php echo ($totle["balance"]); ?>" id="counts">
	        </li>
	        <li class="f-line clearfix">
	          	<label class="f-label">提取金额</label>
	          	<input id="n-money" class="f-input" type="text" placeholder="请输入金额" name="bpprice">
	        </li>
      	</ul>
      	<?php if(!empty($binfo)): if($binfo['bankname'] == ''): ?><ul class="form-box">
			        <li class="f-line clearfix">
			        ...
			        <li class="f-line clearfix">
			          <label class="f-label">省份</label>
			          <select id="selProvince" onChange="provinceChange();" class="f-select" name="province">
			        ...
			        <li class="f-line clearfix">
			          ...
			          <select id="selCity" class="f-select" name="city">
			            <option value="0">请选择城市</option>
			          </select>
			        </li>
			        <li class="f-line clearfix">
			          <label class="f-label">银行支行</label>
			          <input id="idNum" class="f-input c-1" type="text" name="branch" placeholder="请输入银行支行名称" value="">
			        </li>
			        <li class="f-line clearfix">
			          <label class="f-label">卡号</label>
			          <input id="cardNum" class="f-input c-1" type="text" name="banknumber" placeholder="请输入借记卡卡号" value="">
			        </li>
			        <li class="f-line clearfix">
			          <label class="f-label">姓名</label>
			          <input id="Name" class="f-input c-1" type="text" placeholder="请输入持卡人姓名" name="busername" value="">
			        </li>
		      	</ul>
      		<?php else: ?>
	      		<ul class="form-box banks">
	      			<li class="f-line clearfix" style="padding: 5px 0px;">
	      				<?php echo ($binfo["busername"]); ?>&nbsp;&nbsp;您的银行卡
	      			</li>
	      			<li class="" style="height: 100px;">
	      				<p class="bname"><?php echo ($binfo["bankname"]); ?></p>
	      				<p><?php echo ($binfo["province"]); echo ($binfo["city"]); echo ($binfo["branch"]); ?></p>
	      				<p class="bankid"><?php echo ($binfo["banknumber"]); ?></p>
	      				<input type="hidden" name="bankname" id="Bank" value="<?php echo ($binfo["bankname"]); ?>"/>
	      				<input type="hidden" name="province" id="selProvince" value="<?php echo ($binfo["province"]); ?>"/>
	      				<input type="hidden" name="city" id="selCity" value="<?php echo ($binfo["city"]); ?>"/>
	      				<input type="hidden" name="branch" id="idNum" value="<?php echo ($binfo["branch"]); ?>"/>
	      				<input type="hidden" name="busername" id="Name" value="<?php echo ($binfo["busername"]); ?>"/>
	      				<input type="hidden" name="banknumber" id="cardNum" value="<?php echo ($binfo["banknumber"]); ?>"/>
	      			</li>
	      		</ul><?php endif; ?>
      	<?php else: ?>
	      	<ul class="form-box">
...
<script type="text/javascript">
    ...
    $(function () {
	    ...
	    $('.b-close1').click(function(){
	        $('.withdraw-box,.mask').hide();
	    });
	    $('.b-close2').click(function(){
	        $('.tixing-box,.mask').hide();
	    });
    ...
    var $wForm = $('#withDraw'),			//form表单
        ...
        $idNum = $('#idNum'),				//支行
        $dPwd = $('#deal-pwd'),	 			//交易密码
		$province=$("#selProvince"),		//省份
    	$city=$("#selCity"),  				//城市
    	$counts=$("#counts").val(),			//账户余额
		...
		$wForm.submit(function(){
          	...
          	if($nMoney.val() =='' ){
            	...
            	$nMoney.focus();
            	return false;
       		}else if($nMoney.val()<100){
       			msg('您提现金额不能低于100！');
            	$nMoney.focus();
            	return false;
       		}
         	else if((parseInt($nMoney.val()))>parseInt($counts)+1){
      			msg('提现金额不能大于账户余额！');
                $nMoney.focus();
                return false;
      		}
            ...
            $.ajax({
                ...
                success: function (data) {
		            ...
		            if(data==1){
            			...
            			msg('操作成功！正在审核中....');
            			//防止重复提交
            			$('.f-sub').attr('disabled',"true");  
						//2秒后跳转代码
						window.setTimeout(function(){ location.href = "<?php echo U('User/memberinfo');?>"; },2000);	
  					}
