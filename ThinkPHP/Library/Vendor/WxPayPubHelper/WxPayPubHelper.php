...
class Common_util_pub
{
	...
	function __construct($config) {
		$config = M('pay')->where(array('type'=>'WxPay'))->find();
		if(isset($config['appid'])){

			$this->APPID = $config['appid'];
			$this->MCHID = $config['pid'];
			$this->KEY = $config['key'];
			...
			//微信公众号身份的唯一标识。审核通过后，在微信发送的邮件中查看
			/*
			$mc=M('pay')->where(array('type'=>'wechat'))->find();
			$wechat=M('wechat')->find();
			$config['APPID'] = $wechat['appid'];
			$config['MCHID'] = $mc['partner'];
			$config['KEY'] = $mc['key'];
			$config['APPSECRET'] = $wechat['appsecret'];	
			
			*/
			$this->JS_API_CALL_URL=="http://".$_SERVER['SERVER_NAME']."/index.php/home/index/notify";
			$this->NOTIFY_URL=='http://'.$_SERVER['SERVER_NAME'].'/index.php/home/index/notify';
			$this->APPID = 'wx3b2270343126c48a';
			//受理商ID，身份标识
			$this->MCHID = '1229000302';
			//商户支付密钥Key。审核通过后，在微信发送的邮件中查看
			$this->KEY = 'c779e44a1bd14bf0bdfc901a1626622e';
			//JSAPI接口中获取openid，审核后在公众平台开启开发模式后可查看
			$this->APPSECRET = 'e0dc33877e2923675962fd8a373881c8 ';	
			//=======【证书路径设置】=====================================
			//证书路径,注意应该填写绝对路径
